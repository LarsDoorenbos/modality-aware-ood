...
class RawFrameDecode:
    ...
    def __call__(self, results):
        ...
        if modality == 'RGB':
            ...
            video_x = results['video']
            video_y = results['video_y']
        else:
        ...
        # cap = cv2.VideoCapture(directory+".mp4")
        #
        # frames = []
        # if cap.isOpened():
        #     while True:
        ...
        for frame_idx in results['frame_inds']:
            ...
            cur_index = frame_idx-1
            if cur_index < 0:
                cur_index = 0
            if cur_index >= frame_num:
                cur_index = frame_num-1

            if modality == 'RGB':
                try:
                    cur_frame = video.get_data(cur_index)
                except:
                    if cur_index == 0:
                        cur_frame = video.get_data(cur_index+1)
                    else:
                        cur_frame = video.get_data(cur_index-1)
                imgs.append(cur_frame)
                ...
                # imgs.append(cur_frame)
                #imgs.append(frames[frame_idx])
            elif modality == 'Flow':
                try:
                    cur_frame_x = video_x.get_data(cur_index)
                except:
                    if cur_index == 0:
                        cur_frame_x = video_x.get_data(cur_index+1)
                    else:
                        cur_frame_x = video_x.get_data(cur_index-1)

                try:
                    cur_frame_y = video_y.get_data(cur_index)
                except:
                    if cur_index == 0:
                        cur_frame_y = video_y.get_data(cur_index+1)
                    else:
                        try:
                            cur_frame_y = video_y.get_data(cur_index-1)
                        except:
                            try:
                                cur_frame_y = video_y.get_data(cur_index-2)
                            except:
                                try:
                                    cur_frame_y = video_y.get_data(cur_index-3)
                                except:
                                    print("directory: ", directory)
                                    print("frame_num: ", frame_num)
                                    cur_frame_y = video_y.get_data(0)

                cur_frame_x = cv2.cvtColor(cur_frame_x, cv2.COLOR_RGB2GRAY)
                cur_frame_y = cv2.cvtColor(cur_frame_y, cv2.COLOR_RGB2GRAY)
                imgs.extend([cur_frame_x, cur_frame_y])
                
                # x_filepath = osp.join(directory,'u/frame_%010d.jpg'%frame_idx)
                #                       #filename_tmpl.format('x', frame_idx))
                # y_filepath = osp.join(directory,'v/frame_%010d.jpg'%frame_idx)
