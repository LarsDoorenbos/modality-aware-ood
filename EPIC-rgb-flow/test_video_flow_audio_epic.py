...
def validate_one_step(model, clip, labels, flow, model_flow, spectrogram, audio_cls_model):
    ...
    return predict, feature, v_predict, f_predict, audio_predict, v_emd, audio_emd, f_emd
...
if __name__ == '__main__':
    ...
    for split in splits:
        print(split)
        pred_list, conf_list, label_list, output_list, feature_list, pred_v_list, pred_a_list, pred_f_list, feat_v_list, feat_a_list, feat_f_list = [], [], [], [], [], [], [], [], [], [], []
        for clip, flow, spectrogram, labels in tqdm(dataloaders[split]):
            output, feature, output_v, output_f, output_a, v_emd, a_emd, f_emd = validate_one_step(model, clip, labels, flow, model_flow, spectrogram, audio_cls_model)
            ...
            pred_a_list.append(output_a.cpu())
            feat_v_list.append(v_emd.cpu())
            feat_f_list.append(f_emd.cpu())
            feat_a_list.append(a_emd.cpu())
        ...
        pred_a_list = torch.cat(pred_a_list).numpy()
        feat_v_list = torch.cat(feat_v_list).numpy()
        feat_f_list = torch.cat(feat_f_list).numpy()
        feat_a_list = torch.cat(feat_a_list).numpy()
        label_list = torch.cat(label_list).numpy().astype(int)
        feature_list = torch.cat(feature_list).numpy()

        if args.far_ood:
            output_name = './saved_files/id_'+args.dataset+'_ood_'+args.ood_dataset+'_far_ood_output_' + args.appen + split + '.npy'
            pred_name = './saved_files/id_'+args.dataset+'_ood_'+args.ood_dataset+'_far_ood_pred_' + args.appen + split + '.npy'
            conf_name = './saved_files/id_'+args.dataset+'_ood_'+args.ood_dataset+'_far_ood_conf_' + args.appen + split + '.npy'
            vconf_name = './saved_files/id_'+args.dataset+'_ood_'+args.ood_dataset+'_far_ood_vconf_' + args.appen + split + '.npy'
            fconf_name = './saved_files/id_'+args.dataset+'_ood_'+args.ood_dataset+'_far_ood_fconf_' + args.appen + split + '.npy'
            aconf_name = './saved_files/id_'+args.dataset+'_ood_'+args.ood_dataset+'_far_ood_aconf_' + args.appen + split + '.npy'
            label_name = './saved_files/id_'+args.dataset+'_ood_'+args.ood_dataset+'_far_ood_label_' + args.appen + split + '.npy'
            feature_name = './saved_files/id_'+args.dataset+'_ood_'+args.ood_dataset+'_far_ood_feature_' + args.appen + split + '.npy'
            v_feature_name = './saved_files/id_'+args.dataset+'_ood_'+args.ood_dataset+'_far_ood_vfeature_' + args.appen + split + '.npy'
            f_feature_name = './saved_files/id_'+args.dataset+'_ood_'+args.ood_dataset+'_far_ood_ffeature_' + args.appen + split + '.npy'
            a_feature_name = './saved_files/id_'+args.dataset+'_ood_'+args.ood_dataset+'_far_ood_afeature_' + args.appen + split + '.npy'
        else:
            ...
            aconf_name = './saved_files/id_'+args.dataset+'_near_ood_aconf_' + args.appen + split + '.npy'
            ...
            a_feature_name = './saved_files/id_'+args.dataset+'_near_ood_afeature_' + args.appen + split + '.npy'
        ...
        np.save(aconf_name, pred_a_list)
        ...
        np.save(a_feature_name, feat_a_list)
