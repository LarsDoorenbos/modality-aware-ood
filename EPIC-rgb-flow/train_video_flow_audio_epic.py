...
def train_one_step(model, clip, labels, flow, model_flow, spectrogram, audio_cls_model, epoch_i):
    ...
    flow_feat = model_flow.module.backbone.get_predict(flow_feat.detach())
    ...
    if args.use_irm:
        ...
        for class_idx in range(num_class):
            ...
            if class_mask.sum() > 0: 
                class_mean = v_emd[class_mask].mean(dim=0)  
                batch_means.append(class_mean)
            else:
                batch_means.append(prototypes[class_idx])  
        batch_means = torch.stack(batch_means)

        for class_idx in range(num_class):
            class_mask = (labels == class_idx)
            if class_mask.sum() > 0:
                class_var = v_emd[class_mask].var(dim=0, unbiased=False)  
                update_speed = torch.clamp(1.0 / (class_var + 1e-6), min=0.01, max=0.5)  
                update_speed = update_speed / class_mask.sum()  
                with torch.no_grad():
    ...
    if args.use_dynamic_a2d:
        ...
        for i in range(len(v_emd)):
            ...
            if epoch_i < 2:
                a2d_ratio = args.a2d_ratio  
            else:
                distance = torch.cosine_similarity(v_emd[i], prototype, dim=-1) 
                distance = torch.clamp(distance, min=-1 + 1e-7, max=1 - 1e-7) 
                a2d_ratio = (1.0 - torch.sigmoid(distance)) * 3.2 
...
if __name__ == '__main__':
    ...
    log_name = "log_video_afv_%s_near_ood_lr_%s_bsz_%s_%s_%s"%(str(args.dataset), str(args.lr), str(args.bsz), str(args.nepochs), args.opt)
    ...
    params = list(model.module.backbone.fast_path.layer4.parameters()) + list(
        model.module.backbone.slow_path.layer4.parameters()) +list(model.module.cls_head.parameters())+list(model_flow.module.backbone.layer4.parameters()) +list(model_flow.module.cls_head.parameters())+list(audio_cls_model.parameters())
    ...
    with open(log_path, "a") as f:
        for epoch_i in range(starting_epoch, args.nepochs):
            ...
            for split in ['train', 'val', 'test']:
                ...
                audio_model.train(split == 'train')
                mlp_cls.train(split == 'train')
                with tqdm.tqdm(total=len(dataloaders[split])) as pbar:
                    for (i, (clip, flow, spectrogram, labels)) in enumerate(dataloaders[split]):
                        if split=='train':
                            predict1, loss = train_one_step(model, clip, labels, flow, model_flow, spectrogram, audio_cls_model, epoch_i)
                        else:
                            predict1, loss = validate_one_step(model, clip, labels, flow, model_flow, spectrogram, audio_cls_model)
