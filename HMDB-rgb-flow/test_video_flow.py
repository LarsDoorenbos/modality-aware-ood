...
from encoders import Encoder, MidEncoder, BigEncoder
...
def validate_one_step(model, clip, labels, flow, model_flow):
    ...
    return predict, feature, v_predict, f_predict, v_emd, f_emd
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ood_datapath', type=str, default='../data/UCF101/', help='datapath')
    ...
    if args.big:
        mlp_cls = BigEncoder(v_dim=v_dim, a_dim=f_dim, hidden_dim=128, out_dim=num_class, num_heads=4, depth=8)
        mlp_cls = mlp_cls.cuda()
    elif args.mid:
        mlp_cls = MidEncoder(v_dim=v_dim, a_dim=f_dim, hidden_dim=128, out_dim=num_class, num_heads=4, depth=8)
        mlp_cls = mlp_cls.cuda()
    else:
    ...
    model.load_state_dict(checkpoint['model_state_dict'], strict=args.dataset!='EPIC')
    model_flow.load_state_dict(checkpoint['model_flow_state_dict'], strict=args.dataset!='EPIC')
    mlp_cls.load_state_dict(checkpoint['mlp_cls_state_dict'], strict=args.dataset!='EPIC')
    ...
    if args.dataset == 'EPIC' and args.far_ood:
        eval_dataset = EPICDOMAIN(split='eval', cfg=cfg, cfg_flow=cfg_flow, datapath=args.ood_datapath, dataset=args.dataset, near_ood=args.near_ood, far_ood=args.far_ood, ood_dataset=args.ood_dataset)
        ...
        if args.near_ood:
            ...
            if args.ood_dataset == 'EPIC':
                dataloaders = {'train': train_dataloader, 'val': val_dataloader, 'test': test_dataloader}
                splits = ['test', 'train', 'val']
                print(len(train_dataset), len(val_dataset), len(test_dataset))
            else:    
                if args.ood_dataset == "HAC":
                    eval_dataset = HACDOMAIN(cfg=cfg, cfg_flow=cfg_flow, datapath='/path/to/HAC/')
                else:
                    eval_dataset = EPICDOMAIN(split='eval', cfg=cfg, cfg_flow=cfg_flow, datapath=args.ood_datapath, dataset=args.dataset, near_ood=args.near_ood, far_ood=args.far_ood, ood_dataset=args.ood_dataset)
                eval_dataloader = torch.utils.data.DataLoader(eval_dataset, batch_size=args.bsz, num_workers=args.num_workers, shuffle=False,
                                                            pin_memory=False, drop_last=False)
                ...
                if args.dataset == "Kinetics":
                    ...
                    splits = ['test', 'train', 'val', 'eval']

                print(len(train_dataset), len(val_dataset), len(test_dataset), len(eval_dataset))
    
    for split in splits:
        st = time.time()
        pred_list, conf_list, label_list, output_list, feature_list, bary_list, pred_fused_list, pred_v_list, pred_f_list, feat_v_list, feat_f_list = [], [], [], [], [], [], [], [], [], [], []
        for clip, spectrogram, labels in tqdm(dataloaders[split]):
            output, feature, output_v, output_f, v_emd, f_emd = validate_one_step(model, clip, labels, spectrogram, model_flow)
            score = torch.softmax(output, dim=1)
            conf, pred = torch.max(score, dim=1)
            output_list.append(output.cpu())
            pred_list.append(pred.cpu())
            pred_v_list.append(output_v.cpu())
            pred_f_list.append(output_f.cpu())
            feat_v_list.append(v_emd.cpu())
            feat_f_list.append(f_emd.cpu())
            conf_list.append(conf.cpu())
            label_list.append(labels.cpu())
            feature_list.append(feature.cpu())

        print(split, time.time() - st)
        output_list = torch.cat(output_list).numpy()
        pred_list = torch.cat(pred_list).numpy().astype(int)
        conf_list = torch.cat(conf_list).numpy()
        pred_v_list = torch.cat(pred_v_list).numpy()
        pred_f_list = torch.cat(pred_f_list).numpy()
        feat_v_list = torch.cat(feat_v_list).numpy()
        feat_f_list = torch.cat(feat_f_list).numpy()
        ...
        if args.far_ood:
            ...
            vconf_name = 'saved_files/id_'+args.dataset+'_ood_'+ args.ood_dataset + '_vconf_' + args.appen + split + '.npy'
            fconf_name = 'saved_files/id_'+args.dataset+'_ood_'+ args.ood_dataset + '_fconf_' + args.appen + split + '.npy'
            ...
            v_feature_name = 'saved_files/id_'+args.dataset+'_ood_'+ args.ood_dataset + '_vfeature_' + args.appen + split + '.npy'
            f_feature_name = 'saved_files/id_'+args.dataset+'_ood_'+ args.ood_dataset + '_ffeature_' + args.appen + split + '.npy'
        elif args.near_ood:
            ...
            vconf_name = 'saved_files/id_'+args.dataset+'_near_ood_vconf_' + args.appen + split + '.npy'
            fconf_name = 'saved_files/id_'+args.dataset+'_near_ood_fconf_' + args.appen + split + '.npy'
            ...
            v_feature_name = 'saved_files/id_'+args.dataset+'_near_ood_vfeature_' + args.appen + split + '.npy'
            f_feature_name = 'saved_files/id_'+args.dataset+'_near_ood_ffeature_' + args.appen + split + '.npy'
        else:
            ...
            vconf_name = 'saved_files/id_'+args.dataset+'_vconf_' + args.appen + split + '.npy'
            fconf_name = 'saved_files/id_'+args.dataset+'_fconf_' + args.appen + split + '.npy'
            ...
            v_feature_name = 'saved_files/id_'+args.dataset+'_vfeature_' + args.appen + split + '.npy'
            f_feature_name = 'saved_files/id_'+args.dataset+'_ffeature_' + args.appen + split + '.npy'

        np.save(output_name, output_list)
        np.save(pred_name, pred_list)
        np.save(conf_name, conf_list)
        np.save(vconf_name, pred_v_list)
        np.save(fconf_name, pred_f_list)
        np.save(label_name, label_list)
        np.save(feature_name, feature_list)
        np.save(v_feature_name, feat_v_list)
        np.save(f_feature_name, feat_f_list)

        print(pred_f_list.shape, pred_v_list.shape)